    ...
    public void actionPerformed(ActionEvent e) {
        ...
        for (int indiceFila = 0; indiceFila < this.numCelulasVerticales; indiceFila++) {
            for (int indiceColumna = 0; indiceColumna < this.numCelulasHorizontales; indiceColumna++) {
                ...
                numVecinasVivas = 0;
                if (this.esCelulaViva(filaAnterior, columnaAnterior)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(filaAnterior, indiceColumna)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(filaAnterior, columnaPosterior)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(indiceFila, columnaAnterior)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(indiceFila, columnaPosterior)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(filaPosterior, columnaAnterior)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(filaPosterior, indiceColumna)) {
                    numVecinasVivas++;
                }
                if (this.esCelulaViva(filaPosterior, columnaPosterior)) {
                    numVecinasVivas++;
                }

                if (this.celulas[indiceFila][indiceColumna].estaViva()) {
                    if (numVecinasVivas < 2 || numVecinasVivas > 3) {
                        this.celulas[indiceFila][indiceColumna].setViva(false);
                    }
                } else if (numVecinasVivas == 3) {
                    this.celulas[indiceFila][indiceColumna].setViva(true);
                }
            }
        }
